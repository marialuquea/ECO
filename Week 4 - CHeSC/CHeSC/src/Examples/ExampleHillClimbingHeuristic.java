...
public class ExampleHillClimbingHeuristic extends HyperHeuristic {
	
	/**
	 * creates a new ExampleHyperHeuristic object with a random seed
	 */
	public ExampleHillClimbingHeuristic(long seed){
		super(seed);
	}
	...
	public void solve(ProblemDomain problem) {
		...
		//System.out.println("initial fitness is " + problem.getFunctionValue(0));
		...
		while (!hasTimeExpired()) {
			...
			// pick a random mutation heuristic
			...
			// A hillclimber only accepts a new solution if it is an improvement on the current on
			if (delta > 0) {
				//there is an improvement so we 'accept' the solution by copying the new solution into memory index 0
				...
				//System.out.println("found new solution " + problem.getBestSolutionValue() + " at iter " + iterations);
			} 
